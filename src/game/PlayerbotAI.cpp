...
uint32 PlayerbotAI::getSpellId(const char* args, bool master) const
{
	...
	//sLog.outError("GetSessionDBCLocale = %d",loc);
...
typedef std::multimap<Unit::spellEffectPair, Aura*> AuraMap;
...
//		m_targetProtect = 0;
    ...
    else
    {
        ...
        if (!itemIds.empty()) {
        	...
        	Object* const pNpc = m_bot->GetObjectByTypeMask(questRewarderGUID, TYPEMASK_CREATURE_OR_GAMEOBJECT);
        	if (!pNpc)
        		return;
