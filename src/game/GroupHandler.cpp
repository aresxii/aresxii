...
void WorldSession::HandleGroupInviteOpcode( WorldPacket & recv_data )
{
	...
	*/
	...
	Group *pTmpGroup = player->GetGroupInvite();
	Player * pTmpPlayer=0;
	if(pTmpGroup)
		pTmpPlayer = sObjectMgr.GetPlayer(pTmpGroup->GetLeaderName());
	bool isBot=false;
    if (pTmpPlayer && pTmpPlayer->GetPlayerbotMgr())
	{
		sLog.outError("Request Party with Bot");
		Player *bot = 0;
		
        for( PlayerBotMap::const_iterator itr=pTmpPlayer->GetPlayerbotMgr()->GetPlayerBotsBegin(); itr!=pTmpPlayer->GetPlayerbotMgr()->GetPlayerBotsEnd(); ++itr )
        {
            bot = itr->second;
			
			if (bot->GetName()==std::string(membername))
			{
				isBot = true;break;
			}
        }
		...
		{
			SendPartyResult(PARTY_OP_INVITE, membername, PARTY_RESULT_OK);
			WorldPacket data2(CMSG_GROUP_ACCEPT,10);
			data2 << uint8(1);
			data2 << membername;
			player->GetSession()->HandleGroupAcceptOpcode(data2);
			sLog.outError("HandleGroupAcceptOpcode DONE");
		}
