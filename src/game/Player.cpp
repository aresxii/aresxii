...
void Player::PrepareGossipMenu(WorldObject *pSource, uint32 menuId)
{
	...
	if(this->isBotGiver())
	{
		Creature *pCTemp = (Creature*)pSource;
        pCTemp->LoadBotMenu(this);
	}
...
void Player::LoadBotMenu(Player *pPlayer)
{
...
std::string Player::GetScriptName() const
{
    return sObjectMgr.GetScriptName(this->GetScriptId());
}

uint32 Player::GetScriptId() const
{
	uint32 nval = GetEntry();
	 
	if(ObjectMgr::GetCreatureTemplate(GetEntry()))
